...
with st.sidebar:
    page=st.radio("Workspace",["Dashboard","Capture","Human Life Model","Chat as Me","Create","Publish","Skills & Services","Data & Privacy"])
...
if page=="Dashboard":
    ...
    if st.button("Ingest social material",disabled=not social):
        ingest_social_export(platform,social); st.success("Social material captured.")
    st.subheader("Voice capture")
    transcript=st.text_area("Paste a voice memo transcript",height=180)
    if st.button("Learn my speaking style",disabled=not transcript):
        _,style=capture_voice_transcript(transcript); st.success("Voice style learned."); st.code(style)
    ...
    if st.button("Capture text",disabled=not text):
        ingest_text(text,"manual","Manual capture"); st.success("Captured.")
    ...
    if st.button("Rebuild model from my captured material"):
        with st.spinner("Modeling…"): rebuild_hlm()
    hlm=db.get_hlm()
    for d in ["identity","world","story","mindset","drive","pattern","growth"]:
        with st.expander(d.title(),expanded=True):
            ...
            if v.get("evidence"): st.caption("Evidence: "+" • ".join(v["evidence"]))
    ...
    if q:
        ...
        with st.chat_message("assistant"):
            a=ask_as_self(q,st.session_state.chat[:-1]); st.markdown(a)
    ...
    st.divider(); st.subheader("Generate media locally")
    ...
    if c1.button("Generate image with local A1111 / Stability Matrix",disabled=not mp):
        try:
            p=generate_image(mp); st.success(p); st.image(p)
        except Exception as e: st.error(str(e))
    if c2.button("Queue ComfyUI video workflow",disabled=not mp):
        try: st.json(queue_comfy_video(mp))
        except Exception as e: st.error(str(e))

elif page=="Publish":
    items=db.list_content(100)
    options={f"{x['kind']} | {x.get('title') or x['id'][:8]}":x for x in items}
    if not options: st.info("Create some content first.")
    else:
        label=st.selectbox("Content",list(options)); item=options[label]
        ...
        st.caption("DRY_RUN=true prevents network posting. Set it false in your local .env when your credentials are ready.")
        c1,c2=st.columns(2)
        if c1.button("Post now"):
            try:
                result=publish(platform,text,media_url or None,title=item.get("title") or "Video",file_path=media_path,privacy="private")
                st.json(result)
            except Exception as e: st.error(str(e))
        when=st.datetime_input("Schedule for",value=datetime.now())
        if c2.button("Schedule"):
            meta=db.jload(item.get("metadata"),{}) or {}
            if media_url: meta["media_url"]=media_url
            with db.connect() as con:
    ...
    if st.button("Save preferences"):
        try:
            for k,v in json.loads(raw).items(): db.set_pref(k,v)
            st.success("Saved.")
        except Exception as e: st.error(str(e))
    st.subheader("Skills")
    with db.connect() as con: skills=[dict(r) for r in con.execute("SELECT * FROM skills").fetchall()]
    st.dataframe(skills,use_container_width=True)
    name=st.text_input("Skill name"); instructions=st.text_area("Skill instructions")
    if st.button("Add skill",disabled=not name or not instructions):
        with db.connect() as con: con.execute("INSERT INTO skills VALUES(?,?,?,1)",(db.uid(),name,instructions))
        st.success("Skill added.")
    st.subheader("Services")
    sname=st.text_input("Service name"); desc=st.text_area("Service description")
    price=st.number_input("Price in dollars",0.0,10000.0,0.0,1.0)
    if st.button("Add service",disabled=not sname):
        with db.connect() as con: con.execute("INSERT INTO services VALUES(?,?,?,?,?,1)",(db.uid(),sname,desc,int(price*100),"[]"))
    ...
    if st.button("Delete everything",type="primary",disabled=confirm!="DELETE EVERYTHING"):
        db.delete_all(); st.success("Local database cleared.")
